...
# Заголовки разделов и вводное слово — на языке ответа: с русскими заголовками
# казахский ответ выходил двуязычным («## Что говорит закон» над казахским текстом).
SECTION_TITLES = {
    'ru': ('Что говорит закон', 'Как это работает на практике', 'На что обратить внимание', 'Итог'),
    'kk': ('Заң не дейді', 'Іс жүзінде қалай жүреді', 'Неге назар аудару керек', 'Қорытынды'),
    'en': ('What the law says', 'How it works in practice', 'What to watch out for', 'Summary'),
}
SHORT_LEAD = {'ru': 'Кратко:', 'kk': 'Қысқаша:', 'en': 'In short:'}

_VOWELS = frozenset('аеёиоуыэюяәөұүіaeiouy')


def _gibberish_answer(query: str) -> str | None:
    """Набор символов без единой гласной («dfsfsd») — просьба написать вопрос словами.

    Модель на такое отвечала полным шаблоном с разделом «Что говорит закон».
    Короткие сокращения («ГК», «ПК») набором символов не считаются.
    """
...
КАК ОТВЕЧАТЬ
1. Отвечайте по делу и без повторов: весь ответ — не длиннее 450 слов (обычно 250–400). Сначала прямой ответ, затем нормы, затем практическая часть — что сделать, куда обратиться, какие сроки и документы. Каждую норму называйте один раз — в разделе о законе; в остальных разделах на неё только ссылайтесь, не пересказывая. Не повторяйте в замечаниях и итоге то, что уже сказано в шагах. Коротко (два-четыре предложения) можно ответить на простой фактический вопрос или приветствие.
2. Основа ответа — фрагменты из блока КОНТЕКСТ. Это выдержки из действующих актов РК, и они главнее ваших общих представлений о праве: если ваши знания расходятся с текстом нормы, верен текст. Каждое утверждение, взятое из фрагмента, снабжайте ссылкой на него: [Источник 1], [Источник 2] — это порядковый номер из заголовка фрагмента «[Источник N: акт, статья …]», а не номер статьи. Номер статьи берите только из текста фрагмента — каждый начинается с заголовка «Статья N. Название». Используйте только фрагменты, которые действительно относятся к вопросу; остальные молча пропускайте — не упоминайте их и не подгоняйте под вопрос. Если к вопросу не относится ни один фрагмент, скажите об этом одной фразой и отвечайте по правилу 4.
...
- первый абзац начинается с «**{lead}**» — прямой ответ в одном-двух предложениях;
- затем разделы с заголовками второго уровня. Заголовки пишите ровно так, слово в слово, ничего к ним не дописывая: «## {t[0]}», «## {t[1]}», «## {t[2]}», «## {t[3]}». В первом — две-четыре нормы, прямо отвечающие на вопрос, каждая отдельным пунктом списка: полужирное название акта и статьи, суть одним предложением, ссылка [Источник N]; во втором — три-шесть шагов нумерованным списком по одному предложению: что сделать, куда обратиться, какие сроки и документы; в третьем — два-три пункта по одному предложению: исключения, частые ошибки, ответственность; последний — одно-два предложения. Раздел, для которого нечего сказать, пропускайте. Никаких примечаний в скобках вроде «если вам нужны…» и норм «на всякий случай»;
- ссылки [Источник N] ставьте только в разделе «{t[0]}». В шагах и замечаниях норма уже названа выше — повторять ссылку не нужно;
...
Приветствие, благодарность, бессмысленный набор символов, вопрос о вас самих, простой фактический вопрос или вопрос не о праве — один-два абзаца без заголовков и списков.
...
1. Всего не больше 450 слов.
2. Заголовки — ровно «## {t[0]}», «## {t[1]}», «## {t[2]}», «## {t[3]}», без добавленных слов.
3. Норм не больше четырёх, шагов не больше шести, замечаний не больше трёх.
4. Ссылки [Источник N] — только в разделе «{t[0]}».
5. Последний раздел — заголовок «## {t[3]}» и одно-два предложения, а не абзац «{t[3]}: …».
6. Весь текст на языке: {LANG_NAMES[lang]}."""
...
# Хвост после номеров — «:»/«;» либо пробел и слово: модель дописывает «[Источник 1: ст. 178]»
# и даже «[Источник 36 из контекста отсутствует]». Цифра или «и» после пробела — не хвост,
# а продолжение перечисления «[Источники 4, 6]», «[Источник 1 и 2]».
_CITE_ONE = r'\[\s*Источник[иа]?\s*([\d\s,и]+?)\s*(?:[:;][^\]]*|\s(?!и\s)[^\d\s\]][^\]]*)?\]'
...
def _small_talk_answer(query: str) -> str | None:
    ...
    if not words or len(words) > 6:
        return None
    for spec in _SMALL_TALK.values():
...
def _identity_answer(query: str) -> str | None:
    ...
    if any(c in q_lower for c in kz_chars) or "kim" in q_lower:
        return ("Мен — Dalel, Қазақстан Республикасының заңнамасы бойынша ЖИ-кеңесші. "
                "Құқық мәселелері бойынша кодекстер мен заңдардың нормаларына сүйеніп жауап беремін. "
                "Сұрағыңызды қойыңыз.")
    if any(c in q_lower for c in "abcdefghijklmnopqrstuvwxyz") and not any(0x0400 <= ord(c) <= 0x04FF for c in query):
        return ("I am Dalel, an AI assistant for the law of Kazakhstan. I answer legal questions "
                "with references to the codes and laws of the Republic. Ask away.")
    return ("Я — Dalel, ИИ-консультант по законодательству Республики Казахстан. "
            "Отвечаю на вопросы о праве с опорой на нормы кодексов и законов. Задайте ваш вопрос.")
...
class ResponseGenerator:
    ...
    def generate_response(self, user_query: str, search_results: List[Dict],
        ...
        canned = _identity_answer(user_query) or _small_talk_answer(user_query) or _gibberish_answer(user_query)
        if canned:
            return {
                'answer': canned,
                'sources': [],
