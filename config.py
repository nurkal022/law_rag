...
class Config:
    ...
    # Генерация документа — это десяток обращений к модели, то есть минуты
    # работы вычислителя на один запрос. Без потолка один пользователь
    ...
    # Сколько фрагментов уходит в ответ. Было 5, и этого не хватало: разбор
    # неверных ответов показал нужные статьи на 8-м и 17-м месте выдачи —
    # до модели они не доходили. Второго прохода, который поднял бы их выше,
    # сейчас нет: /v1/rerank есть у BGE-M3, но не у OpenAI. Пока его нет,
    # запас берём глубиной выдачи; в окно gpt-4o-mini это укладывается легко.
