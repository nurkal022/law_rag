...
/**
 * Лист документа на клиенте — общий для договоров и законопроектов.
 *
 ...
 * видит его в преамбуле готового документа.
 *
 * Расхождение с сервером здесь допустимо только в мелочах оформления: после
 * запуска составления лист приходит с сервера и заменяет этот.
 *
 * У законопроекта сторон нет, поэтому всё, что касается сторон и преамбулы,
 * на пустом passport.parties само сворачивается в ничто — отдельной ветки
 * «документ без сторон» здесь нет и быть не должно.
 */
...
export function buildPreviewTree(
  ...
  values: Record<string, unknown>,
  lang: DocLang,
  /* Вид документа паспорт не сообщает, а лист по нему различает оформление,
     поэтому его передаёт экран, который точно знает, что составляет. */
  kind: DocTree['meta']['kind'] = 'contract',
): DocTree {
